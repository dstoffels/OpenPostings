...
const ANDROID_STATUS_BAR_TOP_OFFSET = IS_ANDROID
	? Math.max(0, Number(StatusBar.currentHeight || 0))
	: 0;
...
function normalizeAtsValue(value) {
	...
	if (
		normalized === 'dayforce' ||
		normalized === 'dayforcehcm' ||
		normalized === 'dayforcehcm.com'
	) {
		return 'dayforcehcm';
	}
	if (normalized === 'jobvitecom' || normalized === 'jobvite.com') return 'jobvite';
	if (
		normalized === 'hibob.com' ||
		normalized === 'hibobcom' ||
		normalized === 'hibob' ||
		normalized === 'careers.hibob.com' ||
		normalized === 'careershibobcom'
	)
		return 'hibob';
	if (
	...
	if (
		normalized === 'jibeapply' ||
		normalized === 'jibeapply.com' ||
		normalized === 'jibeapplycom'
	) {
		return 'jibeapply';
	}
	...
	if (
		normalized === 'softgarden' ||
		normalized === 'softgarden.io' ||
		normalized === 'softgardenio'
	) {
		return 'softgarden';
	}
	if (
	...
	if (normalized === 'agilehr.com' || normalized === 'agilehrcom' || normalized === 'agilehr')
		return 'agilehr';
	if (normalized === 'avature' || normalized === 'avature.net' || normalized === 'avaturenet')
		return 'avature';
	if (
		normalized === 'comeet' ||
		normalized === 'comeet.com' ||
		normalized === 'comeetcom' ||
		normalized === 'www.comeet.com' ||
		normalized === 'wwwcomeetcom'
	)
		return 'comeet';
	...
	if (normalized === 'governmentjobscom' || normalized === 'governmentjobs.com')
		return 'governmentjobs';
	if (
		normalized === 'policeappcom' ||
		normalized === 'policeapp.com' ||
		normalized === 'www.policeapp.com' ||
		normalized === 'policeapp'
	)
		return 'policeapp';
	if (
		normalized === 'usajobsgov' ||
		normalized === 'usajobs.gov' ||
		normalized === 'www.usajobs.gov' ||
		normalized === 'usajobs'
	)
		return 'usajobs';
	if (
		normalized === 'k12jobspotcom' ||
		normalized === 'k12jobspot.com' ||
		normalized === 'www.k12jobspot.com' ||
		normalized === 'api.k12jobspot.com' ||
		normalized === 'k12jobspot'
	)
		return 'k12jobspot';
	if (
		normalized === 'schoolspringcom' ||
		normalized === 'schoolspring.com' ||
		normalized === 'www.schoolspring.com' ||
		normalized === 'api.schoolspring.com' ||
		normalized === 'schoolspring'
	)
		return 'schoolspring';
	if (
	...
	if (
		normalized === 'recruitcrmiocom' ||
		normalized === 'recruitcrm.io' ||
		normalized === 'recruitcrmio'
	)
		return 'recruitcrm';
	if (
		normalized === 'rippling.com' ||
		normalized === 'ripplingcom' ||
		normalized === 'ats.rippling.com' ||
		normalized === 'atsripplingcom'
	) {
		return 'rippling';
	}
	...
	if (
		normalized === 'careerspage' ||
		normalized === 'careerspage.io' ||
		normalized === 'careerspageio'
	) {
		return 'careerspage';
	}
	if (
...
function normalizeSyncEnabledAts(
	value,
	fallback = DEFAULT_ATS_FILTER_OPTIONS.map((option) => option.value),
) {
...
function toFormSyncServiceSettings(value) {
	...
	const syncEnabledAts = normalizeSyncEnabledAts(
		source.sync_enabled_ats,
		defaults.sync_enabled_ats,
	);
	const minValue = normalizeAtsRequestQueueConcurrency(
		source.min_ats_request_queue_concurrency || defaults.min_ats_request_queue_concurrency,
	);
	const maxValue = normalizeAtsRequestQueueConcurrency(
		source.max_ats_request_queue_concurrency || defaults.max_ats_request_queue_concurrency,
	);
...
const PERSONAL_INFORMATION_FIELDS = [
	...
	{
		key: 'projects_portfolio_file_path',
		label: 'Projects Portfolio File Path',
		placeholder: 'C:\\Users\\You\\Documents\\projects.pdf',
	},
	{
		key: 'certifications_folder_path',
		label: 'Certifications Folder Path',
		placeholder: 'C:\\Users\\You\\Documents\\certifications',
	},
...
function toFormPersonalInformation(value) {
	...
	for (const field of PERSONAL_INFORMATION_FIELDS) {
		if (field.key === 'age' || field.key === 'years_of_experience') {
			...
			formValue[field.key] =
				numericValue === null || numericValue === undefined || Number(numericValue) === 0
					? ''
					: String(numericValue);
			continue;
		}
...
function toFormMcpSettings(value) {
	...
	return {
		...
		dry_run_only:
			source.dry_run_only === undefined ? defaults.dry_run_only : Boolean(source.dry_run_only),
		...
		preferred_remote: ['remote', 'hybrid', 'non_remote'].includes(source.preferred_remote)
			...
			: 'all',
		preferred_industries: Array.isArray(source.preferred_industries)
			? source.preferred_industries.filter(Boolean)
			: [],
		preferred_regions: Array.isArray(source.preferred_regions)
			? source.preferred_regions.filter(Boolean)
			: [],
		preferred_countries: Array.isArray(source.preferred_countries)
			? source.preferred_countries.filter(Boolean)
			: [],
		preferred_states: Array.isArray(source.preferred_states)
			? source.preferred_states.filter(Boolean)
			: [],
		preferred_counties: Array.isArray(source.preferred_counties)
			? source.preferred_counties.filter(Boolean)
			: [],
		...
		<View style={[styles.card, menuOpen ? styles.cardMenuOpen : null]}>
			<View style={styles.postingCardTopRow}>
				<Pressable onPress={onOpenPosting} style={styles.postingCardMainPressArea}>
					...
					{postingCompensationLabel ? (
						<Text style={styles.postingCompensation}>{postingCompensationLabel}</Text>
					) : null}
					{shouldRenderDescription ? (
						<Text style={styles.postingDescription}>{postingDescriptionLabel}</Text>
					) : null}
					{isApplied ? <Text style={styles.postingAppliedNotice}>{appliedByLabel}</Text> : null}
				...
				<View style={styles.postingCardMenuAnchor}>
					...
					{menuOpen ? (
						<View style={[styles.postingCardMenu, { zIndex: 99999, elevation: 99999 }]}>
							...
							>
								<Text style={styles.postingCardMenuItemText}>
									{isIgnoring ? 'Ignoring...' : 'Ignore Job Posting'}
								</Text>
							</Pressable>
							...
							>
								<Text
									style={[
										styles.postingCardMenuItemText,
										styles.postingCardMenuItemTextDestructive,
									]}
								>
									{isBlockingCompany
										? 'Blocking company...'
...
function DrawerItem({ label, selected, onPress }) {
	return (
		...
		>
			<Text style={[styles.drawerItemText, selected ? styles.drawerItemTextSelected : null]}>
				{label}
			</Text>
		</Pressable>
	);
			...
			{open ? (
				<View style={styles.dropdownPanel}>
					<TextInput
						...
						placeholderTextColor={IS_WEB ? '#64748b' : '#9aa5b1'}
						autoCapitalize="none"
					/>
					...
					>
						...
						>
							<Text
								style={[
									styles.dropdownOptionLabel,
									selected === 'all' ? styles.dropdownOptionLabelSelected : null,
								]}
							>
								{anyLabel}
							</Text>
...
export default function App() {
		...
		if (backgroundService && !backgroundService.isRunning()) {
			(async () => {
				...
				await backgroundService.start(
					runAndroidBackendForegroundTask,
					ANDROID_BACKEND_NOTIFICATION_OPTIONS,
				);
			})().catch((errorValue) => {
				if (disposed) return;
				const message = String(errorValue?.message || errorValue);
	...
	const visibleCountryOptions = useMemo(() => {
		...
		return (postingFilterOptions.countries || []).filter((country) =>
			selectedRegions.includes(String(country?.region || '')),
		);
	...
	const visibleCountyOptions = useMemo(() => {
		...
		return (postingFilterOptions.counties || []).filter((county) =>
	...
	const syncAtsOptions = useMemo(() => {
		const labelByValue = new Map(
			(postingFilterOptions.ats || []).map((option) => [
				String(option?.value || ''),
				String(option?.label || ''),
			]),
		);
	...
	const statusText = useMemo(() => {
		...
		const excludedByDate = Number(
			status.excluded_during_sync_by_posting_date ??
				summary.excluded_during_sync_by_posting_date ??
				0,
		);
		...
		const syncEnabledCompanies = Number(
			status.sync_enabled_company_count ?? summary.sync_enabled_company_count ?? 0,
		);
		...
		async (q, options = {}) => {
			...
			try {
				...
				if (normalizedItems.length >= FRONTEND_POSTINGS_FETCH_LIMIT) {
					queueFrontendLog(
						'warn',
						'postings_limit_reached',
						'Postings payload reached frontend fetch limit.',
						{
	...
	const handleSaveSyncSettings = useCallback(async () => {
		...
		const statusLabel = syncSettings.autoSyncEnabled
			? `enabled every ${intervalLabel} ${networkScope}`
			: 'disabled';
	...
	const handleMigrateFromDatabase = useCallback(async () => {
		...
		try {
			...
			if (summary?.selected?.personal_information) {
				messageParts.push(
					`Personal info: ${summary.personal_information_copied ? 'copied' : 'not found'}`,
				);
			}
	...
	const handleSaveMcpSettings = useCallback(async () => {
		...
		try {
			...
			setMcpSettingsNotice(
				`MCP settings saved. ${preview?.count || 0} candidate postings currently match.`,
			);
		} catch (e) {
			setError(String(e.message || e));
		...
		async (posting) => {
			...
			try {
				...
				setApplicationsNotice(
					`Blocked "${companyName}". Postings from this company are now hidden.`,
				);
			} catch (e) {
				setError(String(e.message || e));
			} finally {
				...
				setApplications((prev) =>
					prev.map((application) =>
						application.id === applicationId
							? normalizeApplicationItem({ ...application, ...item })
							: application,
					),
				);
			}
								...
								{remoteFilterOptions.map((option) => {
									...
									return (
										...
										>
											<Text
												style={[
													styles.remoteFilterChipText,
													selected ? styles.remoteFilterChipTextActive : null,
												]}
											>
												{option.label}
											</Text>
			...
			{initializing && postings.length === 0 ? (
				...
				<FlatList
					...
					keyExtractor={(item, index) =>
						String(item?.job_posting_url || item?._row_fallback_key || `posting-${index}`)
					}
					renderItem={({ item }) => (
						<PostingCard
							item={item}
	...
	const renderApplicationsPage = () => (
		<ScrollView contentContainerStyle={styles.settingsContent}>
			<View style={styles.settingsCard}>
				...
				{applicationsNotice ? (
					<Text style={styles.settingsNotice}>{applicationsNotice}</Text>
				) : null}
				{applicationsLoading ? (
				...
				{applications.map((application) => {
					...
					const appliedByLabel = sanitizeDisplayText(
						application?.applied_by_label,
						...
						<View key={application.id} style={styles.applicationCard}>
							...
							<View style={styles.applicationActionsRow}>
								<View style={styles.applicationStatusWrap}>
									<Pressable
										onPress={() =>
											setOpenApplicationStatusForId((prev) =>
												...
												<Pressable
													...
													style={[
														...
														application.status === status
															? styles.applicationStatusMenuItemActive
															: null,
													]}
												>
													<Text
														style={[
															...
															application.status === status
																? styles.applicationStatusMenuItemTextActive
																: null,
														]}
													>
														{status}
								...
								<Pressable
									...
									disabled={isDeleting}
									style={[
										styles.applicationDeleteBtn,
										isDeleting ? styles.applicationDeleteBtnDisabled : null,
									]}
								>
									<Text style={styles.applicationDeleteBtnText}>
					...
					<>
						...
						<Pressable
							...
							style={[
								...
								settingsSaving ? styles.settingsSaveButtonDisabled : null,
							]}
						>
			...
			<View style={styles.settingsCard}>
				...
				<View style={styles.formGroup}>
					...
					<Text style={styles.settingsInlineHint}>
						...
						{formatSyncIntervalLabel(DEFAULT_SYNC_INTERVAL_SECONDS)}). Minimum:{' '}
						{MIN_SYNC_INTERVAL_SECONDS} seconds.
					</Text>
					{Platform.OS !== 'android' ? (
						<Text style={styles.settingsInlineHint}>
							Wi-Fi-only sync is inactive on web and Windows.
						</Text>
					) : null}
				</View>
				...
				<View style={styles.formGroup}>
					...
					<TextInput
						...
						placeholderTextColor={IS_WEB ? '#64748b' : '#9aa5b1'}
					/>
					{syncServiceSettingsLoading ? (
						<ActivityIndicator size="small" style={styles.settingsLoader} />
					) : null}
					<Text style={styles.settingsInlineHint}>
						Range: {syncServiceSettings.min_ats_request_queue_concurrency} to{' '}
						{syncServiceSettings.max_ats_request_queue_concurrency}. Higher values can increase
						throughput but may cause more 429 responses.
					</Text>
					<Text style={styles.settingsInlineHint}>
						Runtime is currently using {syncServiceSettings.active_ats_request_queue_concurrency}.
						This will take effect next time you stop and restart the sync service.
					</Text>
				</View>
				...
				<View style={styles.formGroup}>
					...
					/>
					<Text style={styles.settingsInlineHint}>
						Range: {syncServiceSettings.min_posting_freshness_hours} to{' '}
						{syncServiceSettings.max_posting_freshness_hours} hours.
					</Text>
					<Text style={styles.settingsInlineHint}>
						Runtime is currently using {syncServiceSettings.active_posting_freshness_hours} hours.
						This applies immediately after saving.
					</Text>
				</View>
				...
				<View style={styles.formGroup}>
					...
					{blockedCompanies.map((company) => {
						const companyName = String(
							company?.company_name || company?.normalized_company_name || '',
						).trim();
						const normalizedCompanyName = normalizeCompanyName(companyName);
						...
						return (
							<View key={`blocked-${normalizedCompanyName}`} style={styles.blockedCompanyRow}>
								...
								<Pressable
									...
									style={[
										styles.blockedCompanyUnblockBtn,
										isUnblocking ? styles.blockedCompanyUnblockBtnDisabled : null,
									]}
								>
									<Text style={styles.blockedCompanyUnblockBtnText}>
				...
				<View style={styles.formGroup}>
					...
					<Text style={styles.settingsInlineHint}>
						Migration is intentionally separated into a modal to avoid accidental taps while saving
						sync settings.
					</Text>
					<Pressable
						...
						style={[
							...
							exportSettingsRunning ? styles.settingsSaveButtonDisabled : null,
						]}
					>
				...
				{syncSettingsNotice ? (
					<Text style={styles.settingsNotice}>{syncSettingsNotice}</Text>
				) : null}
				...
				<Pressable
					...
					style={[
						styles.settingsSaveButton,
						syncServiceSettingsSaving ? styles.settingsSaveButtonDisabled : null,
					]}
				>
					<Text style={styles.settingsSaveButtonText}>
						{syncServiceSettingsSaving ? 'Saving...' : 'Save Sync Settings'}
					</Text>
				</Pressable>
			</View>
			...
			>
				<View style={styles.modalOverlay}>
					...
					<View style={styles.modalCard}>
						...
						<Text style={styles.settingsInlineHint}>
							Imports selected data from another SQLite database file. The Companies table is never
							modified.
						</Text>
						...
						>
							<View style={styles.settingsCheckboxList}>
								{[
									...
									{ key: 'blocked_companies', label: 'Blocked Companies' },
									{
										key: 'applications',
										label:
											'Applications (includes application_attribution and posting_application_state)',
									},
								]
							...
							{migrationNotice ? (
								<Text style={styles.settingsNotice}>{migrationNotice}</Text>
							) : null}
	...
	const renderMcpSettingsPage = () => (
		<ScrollView contentContainerStyle={styles.settingsContent}>
			<View style={styles.settingsCard}>
				...
				<Text style={styles.settingsDescription}>
					Configure agent behavior, preferences, and a dedicated agent login email/password used for
					account creation and MFA.
				</Text>
				...
				<Pressable
					...
					style={[
						styles.settingsSaveButton,
						mcpSettingsSaving ? styles.settingsSaveButtonDisabled : null,
					]}
				>
					<Text style={styles.settingsSaveButtonText}>
						{mcpSettingsSaving ? 'Saving...' : 'Save MCP Settings'}
					</Text>
				</Pressable>
			</View>
		</ScrollView>
	...
	const renderActivePage = () => {
		...
		if (effectiveActivePage === PAGE_KEYS.SETTINGS_APPLICANTEE)
			return renderApplicanteeSettingsPage();
	...
	return (
		<SafeAreaView style={styles.container}>
			<View
				style={[
					styles.header,
					IS_ANDROID ? { paddingTop: 12 + ANDROID_STATUS_BAR_TOP_OFFSET } : null,
				]}
			>
...
const styles = StyleSheet.create({
	container: {
		flex: 1,
		backgroundColor: IS_WEB ? '#0f172a' : '#f4f6f8',
	},
	header: {
	...
	hamburgerButton: {
		...
		borderRadius: 10,
		borderWidth: 1,
		borderColor: IS_WEB ? '#334155' : '#d3dbe4',
		backgroundColor: IS_WEB ? '#1e293b' : '#ffffff',
	...
	hamburgerIcon: {
		fontSize: 20,
		fontWeight: '700',
		color: IS_WEB ? '#f8fafc' : '#102a43',
	},
	title: {
		fontSize: 28,
		fontWeight: '700',
		color: IS_WEB ? '#f8fafc' : '#14213d',
	},
	subtitle: {
		fontSize: 14,
		color: IS_WEB ? '#94a3b8' : '#4f5d75',
		marginTop: 4,
	},
	pageTitle: {
		marginTop: 10,
		fontSize: 13,
		color: IS_WEB ? '#cbd5e1' : '#334e68',
		fontWeight: '600',
	},
	small: {
		fontSize: 11,
		color: IS_WEB ? '#94a3b8' : '#7a8798',
		marginTop: 2,
	},
	controls: {
		flexDirection: 'row',
	...
	postingsFiltersToggleBtn: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#334155' : '#c6ceda',
		backgroundColor: IS_WEB ? '#1e293b' : '#ffffff',
		borderRadius: 10,
	...
	postingsFiltersToggleText: {
		color: IS_WEB ? '#f8fafc' : '#334e68',
		fontWeight: '600',
		fontSize: 12,
	},
	postingsFiltersClearBtn: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#334155' : '#dbe2ea',
		borderRadius: 10,
	...
	dropdownTrigger: {
		...
		borderColor: IS_WEB ? '#334155' : '#c6ceda',
		borderRadius: 10,
		backgroundColor: IS_WEB ? '#0f172a' : '#f8fafc',
		paddingHorizontal: 12,
		paddingVertical: 10,
		flexDirection: 'row',
		justifyContent: 'space-between',
	...
	dropdownSearch: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#475569' : '#c6ceda',
		borderRadius: 10,
		backgroundColor: IS_WEB ? '#0f172a' : '#ffffff',
		color: IS_WEB ? '#f8fafc' : '#102a43',
		height: 40,
		paddingHorizontal: 10,
	},
	...
	dropdownOption: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#334155' : '#edf2f7',
		borderRadius: 8,
		paddingHorizontal: 10,
		paddingVertical: 8,
		marginBottom: 6,
	...
	dropdownOptionSelected: {
		borderColor: IS_WEB ? '#10b981' : '#0b6e4f',
		backgroundColor: IS_WEB ? '#064e3b' : '#e8f6ef',
	},
	dropdownOptionDisabled: {
		borderColor: IS_WEB ? '#1e293b' : '#e4e7eb',
		backgroundColor: IS_WEB ? '#0f172a' : '#f5f7fa',
	},
	dropdownOptionLabel: {
		color: IS_WEB ? '#e2e8f0' : '#334e68',
		fontSize: 12,
	},
	dropdownOptionLabelSelected: {
		color: IS_WEB ? '#34d399' : '#0b6e4f',
		fontWeight: '700',
	},
	dropdownOptionLabelDisabled: {
		color: IS_WEB ? '#475569' : '#9aa5b1',
	},
	dropdownEmpty: {
		color: IS_WEB ? '#94a3b8' : '#7a8798',
		fontSize: 12,
	...
	remoteFilterChipActive: {
		borderColor: IS_WEB ? '#38bdf8' : '#102a43',
		backgroundColor: IS_WEB ? '#0284c7' : '#102a43',
	},
	...
	search: {
		...
		borderColor: IS_WEB ? '#475569' : '#c6ceda',
		borderRadius: 10,
		backgroundColor: IS_WEB ? '#1e293b' : '#fff',
		color: IS_WEB ? '#f8fafc' : '#102a43',
		paddingHorizontal: 12,
		height: 42,
	},
	syncBtn: {
		backgroundColor: IS_WEB ? '#059669' : '#0b6e4f',
		borderRadius: 10,
	...
	postingDescription: {
		...
		color: IS_WEB ? '#cbd5e1' : '#334e68',
		lineHeight: 18,
		backgroundColor: IS_WEB ? '#0f172a' : '#f4f7fb',
		borderWidth: 1,
		borderColor: IS_WEB ? '#334155' : '#dbe3ec',
		borderRadius: 8,
		paddingHorizontal: 10,
		paddingVertical: 8,
	},
	...
	postingCardMenuItemDestructive: {
		borderColor: IS_WEB ? '#7f1d1d' : '#f4d4d4',
		backgroundColor: IS_WEB ? '#450a0a' : '#fff4f4',
	},
	...
	applicationCard: {
		...
		borderRadius: 10,
		padding: 10,
		backgroundColor: IS_WEB ? '#1e293b' : '#fdfefe',
	},
	...
	applicationDeleteBtn: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#dc2626' : '#d13a3a',
		borderRadius: 8,
		backgroundColor: IS_WEB ? '#dc2626' : '#d13a3a',
		justifyContent: 'center',
		alignItems: 'center',
		paddingHorizontal: 10,
		minWidth: 84,
	...
	textField: {
		...
		borderColor: IS_WEB ? '#475569' : '#c6ceda',
		borderRadius: 10,
		backgroundColor: IS_WEB ? '#0f172a' : '#fff',
		color: IS_WEB ? '#f8fafc' : '#102a43',
		paddingHorizontal: 12,
		height: 42,
	},
	...
	modalOverlay: {
		...
		backgroundColor: IS_WEB ? 'rgba(0, 0, 0, 0.75)' : 'rgba(16, 42, 67, 0.45)',
		justifyContent: 'center',
		alignItems: 'center',
		paddingHorizontal: 14,
	},
	...
	blockedCompanyUnblockBtn: {
		borderWidth: 1,
		borderColor: IS_WEB ? '#059669' : '#0b6e4f',
		borderRadius: 8,
		backgroundColor: IS_WEB ? '#059669' : '#0b6e4f',
		paddingVertical: 7,
		paddingHorizontal: 10,
	...
	drawerBackdrop: {
		flex: 1,
		backgroundColor: IS_WEB ? 'rgba(0, 0, 0, 0.65)' : 'rgba(16, 42, 67, 0.25)',
	},
	drawerPanel: {
		position: 'absolute',
		left: 0,
		...
		backgroundColor: IS_WEB ? '#1e293b' : '#ffffff',
		borderRightWidth: 1,
		borderRightColor: IS_WEB ? '#334155' : '#dbe2ea',
		paddingTop: 58,
		paddingHorizontal: 12,
	},
	drawerHeading: {
