...
function extractAshbyJobPostingsFromHtml(pageHtml) {
	...
	const jobPostings = Array.isArray(appData?.jobBoard?.jobPostings)
...
function extractAshbyPostingDate(posting) {
	...
	const publishedDate = String(posting?.publishedDate || posting?.publishedAt || '').trim();
...
async function collectPostingsForAshbyCompany(company, options = {}) {
	...
	if (!Array.isArray(jobPostings) || jobPostings.length === 0) {
		...
		jobPostings = Array.isArray(response?.data?.jobBoard?.jobPostings)
	...
	for (const posting of jobPostings) {
		...
		if (shouldDownloadDescriptions || !location) {
			try {
				const details = await fetchAshbyJobPostingDetails(
					config.organizationHostedJobsPageName,
					jobId,
				);
...
function extractAshbyLocationName(posting) {
	...
	const names = [];
	const primary = clean(
		posting?.locationName || posting?.locationExternalName || posting?.location?.name,
	);
	if (primary) names.push(primary);

	const secondaryNames = Array.isArray(posting?.secondaryLocationNames)
	...
	for (const location of alternateSecondary) {
		const name = clean(
			location?.locationName || location?.name || location?.displayName || location,
		);
		if (!name) continue;
		if (names.some((existing) => existing.toLowerCase() === name.toLowerCase())) continue;
		names.push(name);
...
module.exports = {
	collectPostingsForAshbyCompany,
	parseAshbySeededCompanySource,
	inferAshbyLocationFromDescription,
};
