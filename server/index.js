const { importSeedDataIfEmpty } = require('./db/seed-data.js');
...
} = require('./services/blocked-companies.js');
const {
	ensurePersonalInformationTable,
	getPersonalInformation,
	upsertPersonalInformation,
} = require('./services/personal-info.js');
...
} = require('./services/mcp.js');
const {
	listApplications,
	createApplication,
	updateApplicationStatus,
	deleteApplicationById,
} = require('./services/applications.js');
const {
	runAtsSync,
	getSyncScopeStats,
	syncStatus,
...
} = require('./services/sync-settings.js');
const {
...
} = require('./services/runtime-context.js');
...
const SEEDED_ATS_OPTIONS = new Set(
	Array.from(ATS_FILTER_OPTIONS).filter(
		(ats) =>
			!DYNAMIC_ATS_OPTIONS.has(
				String(ats || '')
					.trim()
					.toLowerCase(),
			),
...
const ATS_LABEL_BY_VALUE = new Map(
	ATS_FILTER_OPTION_ITEMS.map((item) => [
		String(item?.value || '')
			.trim()
			.toLowerCase(),
...
async function initDb() {
  ...
  `);
	...
	await importSeedDataIfEmpty(db);
}
...
async function ensurePostingsTable() {
	...
	const requiredPresent = Array.from(requiredColumns).every((column) =>
	...
	if (!existingColumns.has('last_seen_epoch')) {
		...
		await db.run(`UPDATE Postings SET last_seen_epoch = ? WHERE last_seen_epoch IS NULL;`, [
			nowEpochSeconds(),
		]);
	}
...
function createServer() {
	...
	app.post('/extension/seeded-source/upsert', async (req, res) => {
		try {
			...
			if (!classification.supported) {
				throw new Error(
					String(
						classification?.message || 'URL does not match a supported seeded ATS company source.',
					),
				);
			}
			...
			const normalizedUrl = normalizeSourceUrlString(
				classification.canonical_url || sourceUrlInput,
			);
			if (!normalizedUrl) {
	...
	app.get('/postings/filter-options', async (req, res) => {
        ...
        `,
			...
			states = stateRows.map((row) => {
				const code = String(row?.state_usps || '')
					.trim()
					.toUpperCase();
				...
				return {
					value: code,
					label: readableName
						? `${code} - ${readableName.replace(/\b\w/g, (c) => c.toUpperCase())}`
						: code,
				};
			});
		} catch {
		...
		try {
          ...
          `,
			...
			counties = countyRows.map((row) => {
				const stateCode = String(row?.state_usps || '')
					.trim()
					.toUpperCase();
		...
		let countries = Array.isArray(locationGeoOptions?.countries)
			? locationGeoOptions.countries
			: [];
	...
	app.get('/mcp/candidates', async (req, res) => {
		...
		const search =
			overrideSearch || (useSettings ? String(settings?.preferred_search || '').trim() : '');
	...
	app.post('/mcp/cover-letter-draft', async (req, res) => {
		...
		const instructions = String(
			req.body?.instructions || settings?.instructions_for_agent || '',
		).trim();
          ...
          `,
			...
			if (!companyName || !positionName) {
				return res.status(400).json({
					ok: false,
					error:
						'company_name and position_name are required (or provide a valid job_posting_url).',
				});
			}
