...
function buildSyncedAndSourceDateLabel(
	firstSeenEpoch,
	lastSeenEpoch,
	sourceDateLabel,
	sourceDatePrefix = 'Source date',
) {
	const syncedDate = formatEpochDateLabel(firstSeenEpoch || lastSeenEpoch);
...
function hasRealSourcePostingDate(rawPostingDate, ats, firstSeenEpoch, lastSeenEpoch) {
	...
	const parsedEpoch = parsePostingDateToEpochSeconds(
		raw,
		Number.isFinite(referenceEpoch) ? referenceEpoch : nowEpochSeconds(),
	);
	if (!parsedEpoch) return false;

	if (
		SYNC_FALLBACK_ISO_ATS.has(
			String(ats || '')
				.trim()
				.toLowerCase(),
		)
...
function getPostingLocationGeoFilterOptions() {
	...
	const countriesByValue = new Map(
		DEFAULT_COUNTRY_FILTER_OPTIONS.map((country) => [country.value, { ...country }]),
	);
	const defaultCountryValues = new Set(
		DEFAULT_COUNTRY_FILTER_OPTIONS.map((country) => country.value),
	);
	...
	const defaultCountriesInOrder = DEFAULT_COUNTRY_FILTER_OPTIONS.map((country) =>
		countriesByValue.get(country.value),
	).filter(Boolean);
	...
	const countries = [...defaultCountriesInOrder, ...dynamicCountries].sort((a, b) => {
		...
		if (aIsDefault && bIsDefault) {
			const aIndex = DEFAULT_COUNTRY_FILTER_OPTIONS.findIndex(
				(country) => country.value === a.value,
			);
			const bIndex = DEFAULT_COUNTRY_FILTER_OPTIONS.findIndex(
				(country) => country.value === b.value,
			);
...
async function listPostingsWithFilters(options = {}) {
	...
	const industryKeys = normalizeStringArray(options?.industries).map((key) =>
		normalizeLikeText(key),
	);
	...
	if (!search && !hasStructuredFilters) {
		if (includeApplied && includeIgnored) {
			rows = await db.all(
				`
          SELECT id, company_name, position_name, job_posting_url, posting_date, job_description, compensation_type, education_levels, pay_min, pay_max, pay_currency, pay_period, pay_raw, location, first_seen_epoch, last_seen_epoch
          FROM Postings
        ...
        `,
			...
			rows = await db.all(
				`
          SELECT p.id, p.company_name, p.position_name, p.job_posting_url, p.posting_date, p.job_description, p.compensation_type, p.education_levels, p.pay_min, p.pay_max, p.pay_currency, p.pay_period, p.pay_raw, p.location, p.first_seen_epoch, p.last_seen_epoch
          FROM Postings p
	...
	const normalizedCompanyNames = Array.from(
		new Set(rows.map((row) => normalizeLikeText(row?.company_name)).filter(Boolean)),
	);
	...
	const enrichedRows = rows.map((row) => {
		...
		const companyAts = normalizedCompanyName
			? companyAtsByNormalizedName.get(normalizedCompanyName)
			: '';
		const ats = normalizeAtsFilterValue(
			companyAts || inferAtsFromJobPostingUrl(row?.job_posting_url),
		);
		const dbLocation = String(row?.location || '').trim() || null;
		const mappedLocation =
			String(postingLocationByJobUrl.get(row?.job_posting_url) || '').trim() || null;
		...
		const hasRealDate = hasRealSourcePostingDate(
			rawPostingDate,
			ats,
	...
	if (search || hasStructuredFilters) {
		items = enrichedRows.filter((row) => {
			...
			const matchesCompensation = rowMatchesCompensationFilter(
...
async function enrichPostingsWithApplicationState(items) {
	...
	const urls = rows.map((row) => String(row?.job_posting_url || '').trim()).filter(Boolean);
	...
	return rows.map((item) => {
		...
		return {
			...
			applied_by_label: applied
				? normalizeAppliedByLabel(state?.applied_by_label, appliedByType)
				: '',
