...
} = require('../helpers/description-filters');
...
const {
	collectPostingsForTheApplicantManagerCompany,
} = require('../ats/theapplicantmanager/service.js');
...
const {
	collectPostingsForGovernmentJobsDynamic,
	GOVERNMENTJOBS_ESTIMATED_COMPANY_COUNT,
} = require('../ats/governmentjobs/service.js');
const {
	collectPostingsForSmartRecruitersDynamic,
	SMARTRECRUITERS_ESTIMATED_COMPANY_COUNT,
	SMARTRECRUITERS_INSERT_EVERY_N_TARGETS,
} = require('../ats/smartrecruiters/service.js');
const {
	collectPostingsForPoliceappDynamic,
	POLICEAPP_ESTIMATED_COMPANY_COUNT,
} = require('../ats/policeapp/service.js');
const {
	collectPostingsForUsajobsDynamic,
	USAJOBS_ESTIMATED_COMPANY_COUNT,
} = require('../ats/usajobs/service.js');
const {
	collectPostingsForK12jobspotDynamic,
	K12JOBSPOT_ESTIMATED_COMPANY_COUNT,
} = require('../ats/k12jobspot/service.js');
const {
...
const {
	collectPostingsForSchoolspringDynamic,
	SCHOOLSPRING_ESTIMATED_COMPANY_COUNT,
} = require('../ats/schoolspring/service.js');
const {
	collectPostingsForEdjoinDynamic,
	EDJOIN_ESTIMATED_COMPANY_COUNT,
} = require('../ats/edjoin/service.js');
const {
	collectPostingsForWebcruiterDynamic,
	WEBCRUITER_ESTIMATED_COMPANY_COUNT,
} = require('../ats/webcruiter/service.js');
const {
	collectPostingsForAcademicJobsOnlineDynamic,
	ACADEMICJOBSONLINE_ESTIMATED_COMPANY_COUNT,
} = require('../ats/academicjobsonline/service.js');
const {
	collectPostingsForCalcareersDynamic,
	CALCAREERS_ESTIMATED_COMPANY_COUNT,
} = require('../ats/calcareers/service.js');
const {
	collectPostingsForCaloppsDynamic,
	CALOPPS_ESTIMATED_COMPANY_COUNT,
} = require('../ats/calopps/service.js');
const {
	collectPostingsForStatejobsnyDynamic,
	STATEJOBSNY_ESTIMATED_COMPANY_COUNT,
} = require('../ats/statejobsny/service.js');
...
async function collectPostingsForCompany(company, options = {}) {
	const atsName = String(company?.ATS_name || '')
		.trim()
		.toLowerCase();
	if (atsName === 'workday') {
	...
	if (
		atsName === 'applicantpro' ||
		atsName === 'applicantpro.com' ||
		atsName === 'applicantprocom'
	) {
	...
	if (
		atsName === 'zoho' ||
		atsName === 'zohorecruit' ||
		atsName === 'zohorecruit.com' ||
		atsName === 'zohorecruitcom'
	) {
	...
	if (
		...
		atsName === 'hibob.com' ||
		atsName === 'hibobcom' ||
		atsName === 'careers.hibob.com' ||
		atsName === 'careershibobcom'
	) {
	...
	if (atsName === 'avature' || atsName === 'avature.net' || atsName === 'avaturenet') {
	...
	if (atsName === 'factorialhr' || atsName === 'factorialhr.com' || atsName === 'factorialhrcom') {
	...
	if (atsName === 'peopleadmin' || atsName === 'peopleadmin.com' || atsName === 'peopleadmincom') {
	...
	if (atsName === 'softgarden' || atsName === 'softgarden.io' || atsName === 'softgardenio') {
	...
	if (
		atsName === 'recruitcrm' ||
		atsName === 'recruitcrm.io' ||
		atsName === 'recruitcrmiocom' ||
		atsName === 'recruitcrmio'
	) {
		return collectPostingsForRecruitCrmCompany(company);
	}
	if (
		atsName === 'rippling' ||
		atsName === 'rippling.com' ||
		atsName === 'ripplingcom' ||
		atsName === 'ats.rippling.com' ||
		atsName === 'atsripplingcom'
	) {
	...
	if (
		atsName === 'governmentjobs' ||
		atsName === 'governmentjobs.com' ||
		atsName === 'governmentjobscom'
	) {
	...
	if (
		atsName === 'policeapp' ||
		atsName === 'policeapp.com' ||
		atsName === 'policeappcom' ||
		atsName === 'www.policeapp.com' ||
		atsName === 'wwwpoliceappcom'
	) {
	...
	if (
		...
		atsName === 'usajobs.gov' ||
		atsName === 'usajobsgov' ||
		atsName === 'www.usajobs.gov' ||
		atsName === 'wwwusajobsgov'
	) {
		return collectPostingsForUsajobsDynamic();
	}
	if (
		atsName === 'k12jobspot' ||
		atsName === 'k12jobspot.com' ||
		atsName === 'k12jobspotcom' ||
		atsName === 'www.k12jobspot.com' ||
		atsName === 'wwwk12jobspotcom' ||
		atsName === 'api.k12jobspot.com' ||
		atsName === 'apik12jobspotcom'
	) {
	...
	if (
		...
		const companyUrl = String(company?.url_string || '')
			.trim()
			.toLowerCase();
	...
	if (
		atsName === 'schoolspring' ||
		atsName === 'schoolspring.com' ||
		atsName === 'schoolspringcom' ||
		atsName === 'api.schoolspring.com' ||
		atsName === 'apischoolspringcom' ||
		atsName === 'www.schoolspring.com' ||
		atsName === 'wwwschoolspringcom'
	) {
		...
		const runSyncWorker = async () => {
			while (true) {
				...
				try {
					...
					const currentlyEnabledAts = new Set(
						normalizeSyncEnabledAts(Array.from(getSyncEnabledAts())),
					);
					...
					for (const posting of postings) {
						...
						const inferredLocation = String(
							inferPostingLocationFromJobUrl(posting?.job_posting_url) || '',
						).trim();
						const existingLocation = String(
							postingLocationByJobUrl.get(posting?.job_posting_url) || '',
						).trim();
...
async function upsertPostingsBatch(postings, seenEpoch) {
	...
	try {
		for (const posting of postings) {
			...
			const normalizedCompensationType = normalizeCompensationType(
				posting?.compensation_type,
				'unknown',
			);
			const compensationType =
				normalizedCompensationType === 'unknown' ? null : normalizedCompensationType;
			...
			const directLocation = String(posting?.location || '').trim();
			const inferredLocation = String(inferPostingLocationFromJobUrl(jobPostingUrl) || '').trim();
			const locationRaw = directLocation || inferredLocation;
